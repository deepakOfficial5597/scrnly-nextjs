    ...
    return (
        <Listbox value={panel.backgroundValue} onChange={changeGradientSelection}>
            <div className="relative w-full h-full">
                <Listbox.Button className="relative h-full w-full border-2 border-background-primary hover:border-2 hover:border-primary-default rounded-md" as="div" style={{background: panel.backgroundType === BACKGROUND_CONFIG.GRADIENT ? canvas?.styles?.background : getDefaultGradient().gradient }} onClick={ reInitializeState }>
                    {
                        active && (
